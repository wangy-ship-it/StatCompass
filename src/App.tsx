...
const modules: Record<string, LazyExoticComponent<ComponentType<ModuleProps>>> = {
  ...
  m3: lazy(() => import('./components/modules/M3CentralLimitTheorem')),
  m4: lazy(() => import('./components/modules/M4ConfidenceIntervals')),
  m5: lazy(() => import('./components/modules/M5BayesianFrequentist')),
  m6: lazy(() => import('./components/modules/M6ExperimentStructure')),
  m7: lazy(() => import('./components/modules/M7MetricsGuardrails')),
  m8: lazy(() => import('./components/modules/M8MetricSensitivity')),
  m9: lazy(() => import('./components/modules/M9SampleSizePower')),
  m10: lazy(() => import('./components/modules/M10EffectSizeMDE')),
  m11: lazy(() => import('./components/modules/M11VarianceReduction')),
  m12: lazy(() => import('./components/modules/M12TestChooser')),
  m13: lazy(() => import('./components/modules/M13ClusterExperiments')),
  m14: lazy(() => import('./components/modules/M14AATesting')),
  m15: lazy(() => import('./components/modules/M15SequentialTesting')),
  m16: lazy(() => import('./components/modules/M16SRMDiagnostics')),
  m17: lazy(() => import('./components/modules/M17ValidityThreats')),
  m18: lazy(() => import('./components/modules/M18NoveltyTimeEffects')),
  m19: lazy(() => import('./components/modules/M19InteractionEffects')),
  m20: lazy(() => import('./components/modules/M20MultipleTesting')),
  m21: lazy(() => import('./components/modules/M21BootstrapPermutation')),
  m22: lazy(() => import('./components/modules/M22LiftCalculator')),
  m23: lazy(() => import('./components/modules/M23RatioMetrics')),
  m24: lazy(() => import('./components/modules/M24PracticalSignificance')),
  m25: lazy(() => import('./components/modules/M25BayesianABTesting')),
  m26: lazy(() => import('./components/modules/M26MultiarmedBandits')),
  m27: lazy(() => import('./components/modules/M27DifferenceInDifferences')),
  m28: lazy(() => import('./components/modules/M28ResultInterpreter')),
  m29: lazy(() => import('./components/modules/M29HeterogeneousEffects')),
  m30: lazy(() => import('./components/modules/M30SimpsonsParadox')),
  m31: lazy(() => import('./components/modules/M31RegressionToMean')),
  m32: lazy(() => import('./components/modules/M32BiasVariance')),
  m33: lazy(() => import('./components/modules/M33CrossValidation')),
  m34: lazy(() => import('./components/modules/M34ModelMetrics')),
  m35: lazy(() => import('./components/modules/M35Calibration')),
  m36: lazy(() => import('./components/modules/M36CumulativeGains')),
  m37: lazy(() => import('./components/modules/M37FeatureImportance')),
  m38: lazy(() => import('./components/modules/M38ModelDrift')),
};
...
export default function App() {
  ...
  const [visited, setVisited] = useState<Set<string>>(() => {
    try {
      return new Set(JSON.parse(localStorage.getItem('sc-visited') || '[]') as string[]);
    } catch {
      return new Set();
    }
  });
  ...
  const sectionOf = (id: string) => sections.find((s) => s.modules.some((m) => m.id === id));
        ...
        <div className="max-w-[800px] mx-auto px-6 md:px-10 py-8 md:py-12 pb-20">
          <ErrorBoundary key={active}>
            <Suspense fallback={<LoadingFallback />}>
              {Comp && (
                <ModuleProvider moduleId={active}>
                  <Comp
                    navigate={navigate}
                    visited={visited}
                    resetVisited={() => {
                      localStorage.removeItem('sc-visited');
                      setVisited(new Set());
                    }}
                  />
                </ModuleProvider>
          ...
          {active !== 'home' && (
            <nav
              className="flex items-center justify-between mt-12 pt-6 border-t border-[var(--color-border-subtle)]"
            ...
            >
              {prevModule ? (
                ...
                >
                  {prevCrossesSection && (
                    <span className="text-[10px] text-[var(--color-accent-faint)] uppercase tracking-wider">
                      {prevSection!.title}
                    </span>
                  )}
...
const searchIndex = allModules.map((m) => {
  ...
  return {
    ...m,
    section: sec?.title || '',
    searchText: (m.label + ' ' + (sec?.title || '')).toLowerCase(),
  };
});
...
function SearchOverlay({ onSelect, onClose }: SearchOverlayProps) {
      ...
      >
        <div className="flex items-center gap-3 px-4 py-3 border-b border-[var(--color-border-subtle)]">
          <svg
            ...
            viewBox="0 0 20 20"
            fill="none"
            className="text-[var(--svg-text-faint)] flex-shrink-0"
            aria-hidden="true"
          >
            <circle cx="9" cy="9" r="6" stroke="currentColor" strokeWidth="1.5" />
            <path
          ...
          <input
            ...
            aria-label="Search modules"
          />
          <kbd className="hidden sm:inline text-[10px] text-[var(--svg-text-faint)] bg-app-glass px-1.5 py-0.5 rounded ring-1 ring-[var(--color-border-subtle)]">
            ESC
          </kbd>
        </div>
        <div ref={listRef} className="max-h-[320px] overflow-y-auto py-2">
          {filtered.length === 0 && (
            <div className="px-4 py-6 text-center text-[13px] text-[var(--svg-text-faint)]">
              No modules found
            </div>
          )}
          {filtered.map((m, i) => (
            ...
            >
              ...
              <span className="text-[10px] text-[var(--svg-text-faint)] uppercase tracking-wider">
                {m.section}
              </span>
            </button>
          ))}
        ...
        <div className="px-4 py-2 border-t border-[var(--color-border-subtle)] flex items-center gap-4 text-[10px] text-[var(--svg-text-faint)]">
          <span>
            <kbd className="bg-app-glass px-1 py-0.5 rounded ring-1 ring-[var(--color-border-subtle)]">
              &uarr;&darr;
            </kbd>{' '}
            navigate
          </span>
