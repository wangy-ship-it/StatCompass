...
export const sePropDiff = (p1, p2, n) =>
  Math.sqrt((p1 * (1 - p1)) / n + (p2 * (1 - p2)) / n);

export const liftCI = (pC, pT, n, alpha = 0.05) => {
  const absLift = pT - pC;
...
export const polyFit = (xs, ys, deg) => {
  const n = xs.length;
  const d = deg + 1;
...
export const polyEval = (coeffs, x) => {
  let y = 0;
  let xp = 1;
...
export const bonferroni = (alpha, nTests) => alpha / nTests;

export const benjaminiHochberg = (pValues, alpha) => {
...
export const nFromMDE = (mde, alpha, power, p) => {
  const za = zInv(alpha);
  const zb = -zInv((1 - power) * 2);
...
export const obfBounds = (stages, alpha) => {
  const za = zInv(alpha);
  return Array.from({ length: stages }, (_, k) => za * Math.sqrt(stages / (k + 1)));
};

export const pocockBounds = (stages, alpha) => {
  // Pocock uses constant boundary — approximate via alpha spending
  const za = zInv(alpha / stages);
  return Array.from({ length: stages }, () => za);
};

export const alphaSpend = (stage, total, alpha, method) => {
  const t = stage / total;
  if (method === 'obf') {
    // O'Brien-Fleming-like spending: 2*(1-Phi(z_alpha/sqrt(t)))
