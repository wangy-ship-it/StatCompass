...
function flatten(node: ReactNode): string {
  ...
  if (React.isValidElement(node)) {
    const kids = (node.props as Record<string, unknown>).children as ReactNode;
    return flatten(kids);
  }
  return String(node);
}
...
function collectElements(node: ReactNode): ReactElement[] {
  ...
  if (React.isValidElement(node)) {
    return [
      node,
      ...collectElements((node.props as Record<string, unknown>).children as ReactNode),
    ];
  }
  return [];
}
...
describe('conceptLinker / processChildren', () => {
  ...
  it('links only the first occurrence of a term', () => {
    ...
    const conceptLinks = elements.filter((el) => (el.props as Record<string, unknown>).moduleId);
  ...
  it('suppresses self-links to the current module', () => {
    ...
    if (typeof result === 'string') {
      ...
      const conceptLinks = collectElements(result).filter(
        (el) => (el.props as Record<string, unknown>).moduleId,
      );
  ...
  it('respects word boundaries for short terms', () => {
    ...
    const links1 = elements1.filter((el) => (el.props as Record<string, unknown>).moduleId);
    ...
    if (typeof result2 === 'string') {
      ...
      const links2 = collectElements(result2).filter(
        (el) => (el.props as Record<string, unknown>).moduleId === 'm8',
      );
