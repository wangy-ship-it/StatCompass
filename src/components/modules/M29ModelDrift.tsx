...
export default function M29ModelDrift() {
  ...
  const setDriftRate = (v: number) => set('driftRate', v);
  const setRetrainFreq = (v: number) => set('retrainFreq', v);
  ...
  const W = 600,
    ...
    pt = 24,
    pb = 36;
  const toX = (m: number) => pl + (m / months) * (W - pl - pr);
  const toY = (v: number) => H - pb - ((v - 0.45) / (1.0 - 0.45)) * (H - pt - pb);
  ...
  const gauss = (x: number, mu: number) => Math.exp(-0.5 * Math.pow((x - mu) / 0.8, 2));
  const xRange = [-3, 3 + Math.max(driftOffset, 0.5)];
  const dToX = (x: number) => dpl + ((x - xRange[0]) / (xRange[1] - xRange[0])) * (DW - dpl - dpr);
  const dToY = (y: number) => DH - dpb - y * (DH - dpt - dpb);

  const makePath = (mu: number) => {
    let path = '';
    for (let i = 0; i <= 100; i++) {
  ...
  const makeFill = (mu: number) => {
    const path = makePath(mu);
    return (
      path + 'L' + dToX(xRange[1]) + ',' + dToY(0) + 'L' + dToX(xRange[0]) + ',' + dToY(0) + 'Z'
    );
  };
  ...
  const perfTooltipLookup = useCallback(
      ...
      return {
        ...
        lines: [
          ...
          { label: 'AUC', value: pt_.perf.toFixed(3), color: colors.indigo },
          {
            ...
            color: degradation > 10 ? colors.red : colors.amber,
          },
        ],
      };
    },
  ...
  return (
    <div>
      ...
      <Desc>
        ...
        behavior shifts, markets change, and what worked last quarter gradually stops working. This
        is why machine learning requires ongoing investment — not just a one-time build.
      </Desc>
      ...
      <ChartBox h={H} tooltipLookup={perfTooltipLookup}>
        ...
        <line
          ...
          opacity={0.4}
        />
        <text x={W - pr + 2} y={toY(0.5) + 3} fill={colors.red} fontSize={7} opacity={0.5}>
          Random
        </text>
        ...
        <line
          ...
          opacity={0.4}
        />
        <text x={W - pr + 2} y={toY(basePerf) + 3} fill={colors.emerald} fontSize={7} opacity={0.5}>
          Deploy
        </text>
      ...
      <ChartBox h={DH}>
        ...
        <text x={DW / 2} y={DH - 4} fill={sv.textFaint} fontSize={9} textAnchor="middle">
      ...
      <div className="flex gap-3 flex-wrap mb-5">
        ...
        <StatBox
          label="Current AUC"
          value={data.currentPerf.toFixed(2)}
          color={data.gapPct > 10 ? colors.red : colors.indigo}
        />
        <StatBox
          label="Degradation"
          value={data.gapPct.toFixed(1) + '%'}
          color={data.gapPct > 10 ? colors.red : colors.amber}
        />
        <StatBox
          label="Since Retrain"
          value={data.monthsSinceRetrain + ' mo'}
          color={sv.textFaint}
        />
      </div>
      ...
      <Insight>
        Every model is a depreciating asset. The initial build is only the beginning — monitoring
        and retraining are ongoing operational costs. Teams that deploy models without monitoring
        plans are flying blind. Use the retrain frequency slider to see the cost of neglect versus
        regular maintenance.
      </Insight>
    </div>
  );
