...
function computeRates(biasType: string, mag: number) {
  const f = mag / 100;
...
function biasExplanation(biasType: string, _mag: number) {
  if (biasType === 'clean') {
    return {
      explanation:
      ...
      fix: 'This is the ideal case. Maintain it by using proper randomization, stable unit IDs, and pre-registered metrics.',
    };
  }
  if (biasType === 'selection') {
    return {
      ...
      fix: 'Use proper randomization on a stable user ID. Stratify by key covariates. Check for pre-experiment balance between groups.',
    };
  }
  if (biasType === 'novelty') {
    return {
      ...
      fix: 'Run tests long enough for novelty to decay (2+ weeks). Use holdback groups. Compare new vs returning users within the treatment.',
    };
  }
  return {
    explanation:
    ...
    fix: 'Use cluster-based randomization (randomize by region, team, etc.) to create buffer zones between groups. Consider switchback designs for marketplace experiments.',
  };
}
...
export default function M13ValidityThreats() {
  ...
  const setBiasType = (v: string) => set('biasType', v);
  const setMag = (v: number) => set('mag', v);
  ...
  const { explanation, fix } = useMemo(() => biasExplanation(biasType, mag), [biasType, mag]);
  ...
  const toX1 = (v: number) => pl + (v / maxRate) * (W - pl - pr);
  const barH = 28;
  const gap = 20;
  const y1 = pt + 20;
  ...
  const allLifts = curvePoints.map((cp) => cp.lift);
  ...
  const tooltipLookup = useCallback(
    (vbX: number) => {
      ...
      const closest = curvePoints.reduce(
        (best: { m: number; lift: number }, pt: { m: number; lift: number }) =>
          Math.abs(pt.m - mClamped) < Math.abs(best.m - mClamped) ? pt : best,
      );
      ...
      return {
        ...
        lines: [
          ...
          {
            label: 'Observed Lift',
            value: (closest.lift >= 0 ? '+' : '') + closest.lift.toFixed(2) + 'pp',
            color: curveColor,
          },
  ...
  return (
    <div>
      ...
      <ChartBox
        h={H1}
        label="Visualization of selection bias showing how non-random assignment distorts group comparisons"
      >
        {/* Title */}
        <text x={W / 2} y={18} fill={liftColor} fontSize={12} textAnchor="middle" fontWeight="700">
          {'Observed Lift: ' +
            (observedLift >= 0 ? '+' : '') +
            observedLift.toFixed(2) +
            'pp   |   True Lift: +' +
            TRUE_LIFT.toFixed(1) +
            'pp'}
        </text>
      ...
      <ChartBox
        h={H2}
        label="Visualization of survivorship and spillover biases showing how they threaten experiment validity"
      ...
      >
        ...
        {[liftMin, (liftMin + liftMax) / 2, liftMax].map((v, i) => (
          <text key={i} x={pl2 - 6} y={toY2(v) + 3} fill={sv.text} fontSize={9} textAnchor="end">
            {v.toFixed(1)}
          </text>
        ))}
        ...
        <line
          ...
          opacity={0.4}
        />
        <text x={W - pr2 + 2} y={toY2(0) + 3} fill={sv.text} fontSize={8} opacity={0.5}>
          0
        </text>
        ...
        <circle cx={dotX} cy={dotY} r={5} fill={curveColor} stroke={sv.appBg} strokeWidth={2} />
      ...
      {biasType === 'novelty' && mag > 0 && (
        <div
          ...
          style={{ background: sv.fillAmberSubtle, border: '1px solid ' + sv.borderAmber }}
        >
          <div
            className="text-[11px] uppercase tracking-widest mb-1 font-bold"
          ...
          <div className="text-[13px] text-[var(--color-text-primary)] leading-relaxed">
            ...
            inflated observed rate of {treatmentObs.toFixed(2)}% will converge toward the true rate
            of {TREATMENT_TRUE.toFixed(1)}% over 2-4 weeks. Always run experiments long enough for
            this decay to stabilize before making ship decisions.
          </div>
        </div>
      )}
      ...
      >
        ...
        <div className="text-sm text-[var(--color-text-primary)] leading-relaxed mb-3">
          {explanation}
        </div>
