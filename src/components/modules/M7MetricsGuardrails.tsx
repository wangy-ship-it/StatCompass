...
export default function M6MetricsGuardrails() {
  ...
  const setThreshold = (v: number) => set('threshold', v);
  const setNGuardrails = (v: number) => set('nGuardrails', v);
    ...
    (vbX: number, vbY: number) => {
      ...
      return {
        ...
        lines: [
          ...
          {
            label: 'Threshold',
            value: '\u00B1' + data.threshold.toFixed(1) + '%',
            color: colors.amber,
          },
  ...
  return (
    <div>
      ...
      <Desc>
        ...
        nothing important broke in the process. But monitoring many guardrails simultaneously
        inflates false alarm rates — even when nothing actually changed. Drag the sliders to see how
        the number of guardrails and threshold width interact with the multiple testing problem.
      </Desc>

      <ChartBox
        h={H}
        label="Metrics dashboard showing primary metric, secondary metrics, and guardrail metrics with their current values and thresholds"
      ...
      >
        ...
        <rect
          x={toX(data.ciLo)}
          y={primaryY}
          width={Math.max(toX(data.ciHi) - toX(data.ciLo), 4)}
          height={barH}
          rx={4}
        ...
        <line
          x1={pl - 10}
          y1={guardrailStartY - 14}
          x2={W - pr + 10}
          y2={guardrailStartY - 14}
          stroke={sv.axis}
          strokeWidth={0.5}
        />
        <text x={16} y={guardrailStartY - 6} fill={sv.text} fontSize={9} fontWeight="600">
          Guardrails
        </text>
          ...
          return (
            <g key={i}>
              ...
              <circle
                ...
                r={3.5}
                fill={barColor}
                stroke={g.tripped ? sv.appBg : 'none'}
                strokeWidth={1}
              />
      ...
      />
      <TechNote>
        The family-wise error rate for k independent tests at level \u03b1 is FWER = 1 - (1 -
        \u03b1)^k. With 5 guardrails at \u03b1 = 0.05, FWER = 22.6%. Bonferroni correction tests
        each at \u03b1/k, guaranteeing FWER \u2264 \u03b1. For correlated guardrails (common in
        practice), Bonferroni is overly conservative — methods like Holm-Bonferroni or
        bootstrap-based joint tests provide tighter control. When guardrail metrics share underlying
        user behavior, their test statistics are correlated, and the true FWER falls below the
