...
export default function M15InteractionEffects() {
  ...
  const setEffectA = (v: number) => set('effectA', v);
  const setEffectB = (v: number) => set('effectB', v);
  const setInteractionPct = (v: number) => set('interactionPct', v);
  const setInterType = (v: string) => set('interType', v);
        ...
        lines: [
          {
            ...
            value: v === 0 ? '0' : (v > 0 ? '+' : '') + v.toFixed(1) + '%',
            color: barColors[bestIdx],
          },
          ...(bestIdx === 3
            ? [
                {
                  label: 'Expected',
                  value: '+' + data.expectedCombined.toFixed(1) + '%',
                  color: colors.red,
                },
              ]
            : []),
  ...
  return (
    <div>
      ...
      <div className="flex justify-center mb-8">
        <div>
          <div className="text-[11px] text-[var(--svg-text-faint)] text-center mb-2 uppercase tracking-wider">
            2x2 Factorial Outcomes
          </div>
          <div className="inline-block">
            {/* Column headers */}
            <div className="flex">
              <div className="w-[80px]" />
              <div className="w-[120px] text-center text-[12px] text-[var(--svg-text-faint)] py-1">
                Control B
              </div>
              <div
                className="w-[120px] text-center text-[12px] font-medium py-1"
            ...
            {/* Row 1: Control A */}
            <div className="flex">
              <div className="w-[80px] flex items-center text-[12px] text-[var(--svg-text-faint)] pr-2 justify-end">
                Control A
              </div>
              <div className="w-[120px] h-[70px] flex items-center justify-center rounded-tl-xl ring-1 ring-[var(--color-border-subtle)] bg-app-glass">
                <span className="text-[18px] font-mono font-bold" style={{ color: sv.text }}>
                  {data.cells.cc.toFixed(1)}
                </span>
              </div>
              <div className="w-[120px] h-[70px] flex items-center justify-center rounded-tr-xl ring-1 ring-[var(--color-border-subtle)] bg-app-glass">
                <span className="text-[18px] font-mono font-bold" style={{ color: colors.emerald }}>
                  {data.cells.cb.toFixed(1)}
                </span>
              </div>
            </div>
            {/* Row 2: Treatment A */}
            <div className="flex">
              <div
                className="w-[80px] flex items-center text-[12px] font-medium pr-2 justify-end"
              ...
              <div className="w-[120px] h-[70px] flex items-center justify-center rounded-bl-xl ring-1 ring-[var(--color-border-subtle)] bg-app-glass">
                <span className="text-[18px] font-mono font-bold" style={{ color: colors.indigo }}>
                  {data.cells.ac.toFixed(1)}
                </span>
              </div>
              <div
                className="w-[120px] h-[70px] flex items-center justify-center rounded-br-xl ring-1 ring-[var(--color-border-subtle)]"
                style={{
                  background:
              ...
              >
                <span className="text-[18px] font-mono font-bold" style={{ color: colors.amber }}>
                  {data.cells.ab.toFixed(1)}
                </span>
              </div>
            </div>
          </div>
      ...
      <ChartBox
        h={H}
        label="Interaction effects between simultaneous experiments, showing how treatments can amplify, cancel, or reverse each other"
      ...
      <div className="flex gap-3 flex-wrap mt-6">
        ...
        <StatBox
          label="Actual A+B"
          value={(data.actualCombined >= 0 ? '+' : '') + data.actualCombined.toFixed(1) + '%'}
          color={colors.amber}
        />
      </div>
      ...
      {data.interactionTerm !== 0 && (
        <div
          className="rounded-xl px-5 py-3 mt-4 text-[14px] text-[var(--svg-text)]"
          style={{
        ...
        >
          <span
            ...
            style={{ color: data.interactionTerm > 0 ? colors.emerald : colors.red }}
          >
            Interaction: {data.interactionTerm > 0 ? '+' : ''}
            {data.interactionTerm.toFixed(2)}%
          </span>
          {' — '}
          {data.interactionTerm > 0
      ...
      />
      <TechNote>
        In a 2x2 factorial design, the interaction term is: AB - A - B + baseline. If treatments are
        independent, this equals zero. Interaction as a percentage of individual effects is a useful
        metric: |interaction| / min(A, B). Values above 20% warrant investigation.
      </TechNote>
      <Insight>
        Most experiment platforms assume treatments are independent and measure them separately.
        This works when experiments affect different parts of the product. But when two experiments
        modify the same user flow, ignoring interactions can lead to shipping a combination that
        actually hurts — even though each individual test looked positive.
      </Insight>
    </div>
  );
}
