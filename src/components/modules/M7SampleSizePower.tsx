...
export default function M7SampleSizePower() {
  ...
  const W = 600,
    ...
    pt = 16,
    pb = 38;
  const toX = (n: number) => pl + ((n - 10) / 490) * (W - pl - pr);
  const toY = (p: number) => H - pb - p * (H - pt - pb);
  ...
  return (
    <div>
      ...
      <ChartBox
        h={H}
        label="Power curve showing how statistical power increases with sample size for the selected effect size and significance level"
      ...
      {n80 && (
        <div className="bg-app-surface rounded-xl p-4 text-center mt-2">
          ...
          <span className="text-xl font-extrabold" style={{ color: colors.amber }}>
            {n80.n}
          </span>
