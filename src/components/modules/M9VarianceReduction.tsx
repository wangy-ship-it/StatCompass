...
export default function M9VarianceReduction() {
  ...
  const setRho = (v: number) => set('rho', v);
  ...
  const data = useMemo(() => {
    ...
    for (let i = 0; i <= nSteps; i++) {
      ...
      powerPoints.push({
        n,
        powOrig: Math.max(0, Math.min(1, powOrig)),
        powAdj: Math.max(0, Math.min(1, powAdj)),
      });
    }
  ...
  const toX = (v: number) => pl + ((v + distRange) / (2 * distRange)) * (W - pl - pr);
  const distSteps = 200;
  ...
  const toY = (v: number) => H1 - pb - (v / maxY) * (H1 - pt - pb);
    ...
    (vbX: number) => {
      ...
      return {
        ...
        lines: [
          ...
          {
            label: 'Original',
            value: (closest.powOrig * 100).toFixed(1) + '%',
            color: colors.indigo,
          },
  ...
  return (
    <div>
      ...
      <div className="text-center mb-6">
        <div className="text-[13px] text-[var(--svg-text-faint)] mb-1">
          Variance Reduction Factor
        </div>
        <div
          className="text-[36px] font-bold font-mono transition-all duration-300"
          style={{ color: colors.emerald }}
        >
          {(1 - rho * rho).toFixed(3)}
        </div>
        <div className="text-[12px] text-[var(--svg-text-faint)]">
          1 - ρ² = remaining variance fraction
        </div>
      </div>
      ...
      <ChartBox
        h={H1}
        label="Distribution comparison showing how CUPED reduces variance by using pre-experiment covariate data"
      ...
      >
        ...
        <line
          ...
          strokeDasharray="6,4"
        />
        <text x={pl + 35} y={pt + 8} fill={colors.indigo} fontSize={10}>
          Original
        </text>
        <line
          x1={pl + 110}
          ...
          strokeWidth={2.5}
        />
        <text x={pl + 135} y={pt + 8} fill={colors.emerald} fontSize={10}>
          CUPED-adjusted
        </text>
      </ChartBox>
      ...
      <ChartBox
        h={H2}
        label="Power curves comparing original and CUPED-adjusted experiments, showing how variance reduction increases power at every sample size"
        tooltipLookup={tooltipPower}
      >
        {/* Grid lines */}
        {[0.2, 0.4, 0.6, 0.8].map((p) => (
          <g key={p}>
            <line
              ...
              strokeWidth={0.5}
            />
            <text x={pl - 8} y={pToY(p) + 3} fill={sv.textFaint} fontSize={9} textAnchor="end">
              {(p * 100).toFixed(0)}%
            </text>
          </g>
        ))}
        ...
        <line
          ...
          opacity={0.6}
        />
        <text x={W - pr + 4} y={pToY(0.8) + 3} fill={colors.amber} fontSize={9}>
          80%
        </text>
        {/* Power curves */}
        <path
        ...
        <text x={W / 2} y={H2 - 6} fill={sv.textFaint} fontSize={10} textAnchor="middle">
          Sample Size
        </text>
        <text
          x={14}
          ...
          fontSize={10}
          textAnchor="middle"
          transform={`rotate(-90,14,${(H2 - pt - pb) / 2 + pt})`}
        >
          Power
        </text>
        {/* Legend */}
      ...
      <Sl
        label="Pre-experiment Covariate Correlation (ρ)"
        value={rho}
        min={0}
        max={0.95}
        step={0.01}
      ...
      <div className="flex gap-3 flex-wrap mt-6">
        ...
        <StatBox
          label="Reduction"
          value={data.reductionPct.toFixed(1) + '%'}
          color={colors.amber}
        />
        <StatBox
          label="N Multiplier"
      ...
      <TechNote>
        ...
        covariate. The variance reduction factor is exactly 1 - ρ², making a sample of size n behave
        like n/(1-ρ²) users. This is mathematically equivalent to regression adjustment.
      </TechNote>
      <Insight>
        CUPED is one of the highest-leverage improvements for an experimentation platform. It does
        not require any changes to experiment design or user allocation — just historical data.
        Teams that implement CUPED typically cut required test durations by 30-50%.
      </Insight>
    </div>
  );
}
