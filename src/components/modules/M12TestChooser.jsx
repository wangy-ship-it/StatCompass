...
const getAssumptions = (nodeId, normal, equalVar, independent) => {
  const warnings = [];
  ...
  // Independence checks
  if (tTests.includes(nodeId) || anovaTests.includes(nodeId) || chiTests.includes(nodeId) || fisherTest.includes(nodeId)) {
    warnings.push({
      label: 'Independence',
      ok: independent,
      msg: independent
...
const renderTwoCurves = () => {
  ...
  const makePath = (mean, std, col) => {
    ...
    d += 'L' + endX.toFixed(1) + ',' + (pad.t + ph) + 'L' + startX.toFixed(1) + ',' + (pad.t + ph) + 'Z';
    return (
      <path
        key={col}
        d={d}
  ...
  return (
    <>
      ...
      <line x1={m1x} y1={m1y} x2={m1x} y2={baseY} stroke={colors.indigo} strokeWidth={1.5} strokeDasharray="4,3" opacity={0.7} />
      <line x1={m2x} y1={m2y} x2={m2x} y2={baseY} stroke={colors.emerald} strokeWidth={1.5} strokeDasharray="4,3" opacity={0.7} />
      <text x={m1x} y={baseY + 14} fill={colors.indigo} fontSize={9} textAnchor="middle" fontWeight="600">
        Group A
      </text>
      <text x={m2x} y={baseY + 14} fill={colors.emerald} fontSize={9} textAnchor="middle" fontWeight="600">
        Group B
      </text>
      {/* Axis line */}
...
const renderRankBars = () => {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    bars.push(
      <rect
        key={'a' + i}
        x={xA} y={pad.t + ph - hA} width={barW} height={hA}
        rx={2}
        fill={colors.indigo} fillOpacity={0.6}
        stroke={colors.indigo} strokeWidth={0.5}
      />
    );
  }
  for (let i = 0; i < 10; i++) {
    ...
    bars.push(
      <rect
        key={'b' + i}
        x={xB} y={pad.t + ph - hB} width={barW} height={hB}
        rx={2}
        fill={colors.emerald} fillOpacity={0.6}
        stroke={colors.emerald} strokeWidth={0.5}
      />
    );
  }
  ...
  return (
    <>
      {bars}
      <line x1={sepX} y1={pad.t} x2={sepX} y2={baseY} stroke={sv.axis} strokeWidth={1} strokeDasharray="3,3" />
      <line x1={pad.l} y1={baseY} x2={W - pad.r} y2={baseY} stroke={sv.axis} strokeWidth={1} />
      <text x={pad.l + (sepX - pad.l) / 2} y={baseY + 14} fill={colors.indigo} fontSize={9} textAnchor="middle" fontWeight="600">
        Group A ranks
      </text>
      <text x={sepX + (W - pad.r - sepX) / 2} y={baseY + 14} fill={colors.emerald} fontSize={9} textAnchor="middle" fontWeight="600">
        Group B ranks
      </text>
      <text x={W - pad.r} y={pad.t + 10} fill={sv.text} fontSize={8} textAnchor="end">
        Rank distributions
...
const renderBoxPlots = () => {
  ...
  const groups = [
    { label: 'Group 1', color: colors.indigo, q1: 0.25, median: 0.40, q3: 0.55, lo: 0.10, hi: 0.70 },
    { label: 'Group 2', color: colors.emerald, q1: 0.40, median: 0.58, q3: 0.72, lo: 0.22, hi: 0.88 },
    { label: 'Group 3', color: colors.amber, q1: 0.50, median: 0.68, q3: 0.82, lo: 0.30, hi: 0.95 },
  ];
  ...
  groups.forEach((g, i) => {
    ...
    elements.push(
      <line key={'w' + i} x1={cx} y1={toY(g.hi)} x2={cx} y2={toY(g.lo)} stroke={g.color} strokeWidth={1.5} opacity={0.5} />
    );
    // Whisker caps
    elements.push(
      <line key={'wt' + i} x1={cx - 10} y1={toY(g.hi)} x2={cx + 10} y2={toY(g.hi)} stroke={g.color} strokeWidth={1.5} opacity={0.5} />
    );
    elements.push(
      <line key={'wb' + i} x1={cx - 10} y1={toY(g.lo)} x2={cx + 10} y2={toY(g.lo)} stroke={g.color} strokeWidth={1.5} opacity={0.5} />
    );
    // Box (Q1 to Q3)
    elements.push(
      <rect
        key={'box' + i}
        x={x} y={toY(g.q3)} width={boxW} height={toY(g.q1) - toY(g.q3)}
        rx={4}
        fill={g.color} fillOpacity={0.12}
        stroke={g.color} strokeWidth={1.5}
      />
    );
    // Median line
    elements.push(
      <line key={'med' + i} x1={x} y1={toY(g.median)} x2={x + boxW} y2={toY(g.median)} stroke={g.color} strokeWidth={2.5} />
    );
    // Label
    elements.push(
      <text key={'lbl' + i} x={cx} y={baseY + 14} fill={g.color} fontSize={9} textAnchor="middle" fontWeight="600">
        {g.label}
      </text>
    );
  });
  ...
  // Axis
  elements.push(
    <line key="axis" x1={pad.l - 10} y1={baseY} x2={W - pad.r + 10} y2={baseY} stroke={sv.axis} strokeWidth={1} />
  );
  elements.push(
...
const renderContingency = () => {
  ...
  const observed = [[45, 15], [30, 10]];
  const obsCols = [
    [colors.indigo, colors.emerald],
    [colors.emerald, colors.indigo],
  ];
  const opacities = [[0.25, 0.15], [0.15, 0.10]];

  const elements = [];

  // "Observed" label
  elements.push(
    <text key="obs-title" x={startX + gridW / 2} y={startY - 6} fill={sv.text} fontSize={9} textAnchor="middle" fontWeight="600">
  ...
  // Row labels
  elements.push(
    <text key="r0" x={startX - 8} y={startY + cellH / 2 + 4} fill={sv.text} fontSize={8} textAnchor="end">Row 1</text>
  );
  elements.push(
    <text key="r1" x={startX - 8} y={startY + cellH + gap + cellH / 2 + 4} fill={sv.text} fontSize={8} textAnchor="end">Row 2</text>
  );
  // Col labels
  elements.push(
    <text key="c0" x={startX + cellW / 2} y={startY + gridH + 14} fill={sv.text} fontSize={8} textAnchor="middle">Col 1</text>
  );
  elements.push(
    <text key="c1" x={startX + cellW + gap + cellW / 2} y={startY + gridH + 14} fill={sv.text} fontSize={8} textAnchor="middle">Col 2</text>
  );
    ...
    for (let c = 0; c < 2; c++) {
      ...
      elements.push(
        <rect
          ...
          rx={6}
          fill={obsCols[r][c]} fillOpacity={opacities[r][c]}
          stroke={obsCols[r][c]} strokeWidth={1} opacity={0.8}
        />
      );
      elements.push(
        <text
          ...
          fill={obsCols[r][c]} fontSize={13} textAnchor="middle" fontWeight="700"
        >
          {observed[r][c]}
        </text>
      );
  ...
  elements.push(
    <text key="exp-title" x={expX + gridW / 2 - 40} y={startY - 6} fill={sv.text} fontSize={9} textAnchor="middle" fontWeight="600">
      Expected
    </text>
  );

  const expected = [[37.5, 22.5], [37.5, 2.5]];
  const miniW = 60, miniH = 28, miniGap = 4;
  for (let r = 0; r < 2; r++) {
    for (let c = 0; c < 2; c++) {
      ...
      elements.push(
        <rect
          key={'exp' + r + c}
          x={x} y={y} width={miniW} height={miniH}
          rx={4}
          fill={sv.axis} fillOpacity={0.4}
          stroke={sv.axis} strokeWidth={1}
        />
      );
      elements.push(
        <text
          key={'expv' + r + c}
          x={x + miniW / 2} y={y + miniH / 2 + 3}
          fill={sv.text} fontSize={10} textAnchor="middle" fontWeight="600"
        >
...
const renderPoisson = () => {
  ...
  for (let k = 0; k <= maxK; k++) {
    ...
    elements.push(
      <rect
        ...
        rx={2}
        fill={colors.indigo} fillOpacity={opacity}
        stroke={colors.indigo} strokeWidth={0.8}
      />
    );
    if (k % 2 === 0 || k <= 6) {
      elements.push(
        <text key={'lbl' + k} x={x + barW / 2} y={baseY + 12} fill={sv.text} fontSize={7} textAnchor="middle">
          {k}
        </text>
      );
    }
  ...
  // Axis
  elements.push(
    <line key="axis" x1={pad.l} y1={baseY} x2={W - pad.r} y2={baseY} stroke={sv.axis} strokeWidth={1} />
  );
  elements.push(
...
function AssumptionsChecker({ nodeId }) {
        ...
        <div className="space-y-2">
          {warnings.map((w, i) => (
            ...
            >
              ...
              <div>
                <span className="font-semibold" style={{ color: w.ok ? colors.emerald : colors.amber }}>
                  {w.label}:
                </span>{' '}
  ...
  return (
    <div>
      ...
      <Desc>
        ...
        statistical test. The right test depends on your outcome type, number of groups, pairing, and
        distributional assumptions.
      </Desc>

      <div className="bg-app-surface rounded-2xl p-6 mb-7 ring-1 ring-[var(--color-border-subtle)]">
        <svg viewBox={'0 0 ' + W + ' ' + svgH} width="100%" style={{ maxHeight: svgH, display: 'block', overflow: 'visible' }}>
          {renderNodesSVG()}
        </svg>
      </div>
      ...
      {!isLeaf && (
        <div className="mb-5">
          <div className="text-[13px] text-[var(--color-text-primary)] font-semibold mb-3">{currentNode.question}</div>
      ...
      {isLeaf && (
        <div className="rounded-xl p-5 mb-5 bg-emerald-400/[0.06] border border-emerald-400/20">
          ...
          <div className="text-lg font-bold text-[var(--color-emerald-text)] mb-2">{currentNode.test}</div>
          <div className="text-sm text-[var(--color-text-primary)] leading-relaxed mb-2">{currentNode.desc}</div>
      ...
      />
      <TechNote>
        This tree covers the most common scenarios. Advanced cases: repeated measures → mixed effects
        models; time-to-event → survival analysis (log-rank, Cox); clustered data → GEE or multilevel
        models; high-dimensional → FDR control with BH procedure.
      </TechNote>
      <Insight>
        Choosing the wrong test doesn't always give wrong results — many tests are robust to moderate
        violations of their assumptions. But using the right test gives you the most statistical power
        to detect real effects with the data you have.
      </Insight>
    </div>
  );
}
