...
export default function M17LiftCalculator() {
  ...
  const barColor = significant ? (result.absLift > 0 ? colors.emerald : colors.red) : colors.indigo;
    ...
    (vbX: number) => {
      ...
      return {
        ...
        lines: [
          ...
          {
            label: 'Lift',
            value: (result.absLift > 0 ? '+' : '') + (result.absLift * 100).toFixed(2) + 'pp',
            color: barColor,
          },
          {
            label: '95% CI',
            value: (result.ciLo * 100).toFixed(2) + ' to ' + (result.ciHi * 100).toFixed(2),
            color: barColor,
          },
          {
            label: 'In CI',
            value: withinCI ? 'Yes' : 'No',
            color: withinCI ? colors.emerald : colors.red,
          },
        ],
  ...
  return (
    <div>
      ...
      <ChartBox
        h={H}
        label="Lift estimate with confidence interval showing the absolute difference between treatment and control conversion rates"
      ...
      <div className="flex gap-3 mb-5 flex-wrap">
        <StatBox
          label="Relative Lift"
          value={(result.relLift > 0 ? '+' : '') + result.relLift.toFixed(1) + '%'}
          color={barColor}
        />
        <StatBox
          label="Absolute Lift"
        ...
        <StatBox
          label="P-value"
          value={result.pValue < 0.0001 ? '< 0.0001' : result.pValue.toFixed(4)}
          color={result.pValue < 0.05 ? colors.emerald : sv.textFaint}
        />
        <StatBox
          label="95% CI"
          value={(result.ciLo * 100).toFixed(2) + ' to ' + (result.ciHi * 100).toFixed(2) + 'pp'}
          color={sv.textFaint}
        />
      </div>
      ...
      />
      <TechNote>
        Lift CI uses the Wald method: SE = √(p₁(1−p₁)/n + p₂(1−p₂)/n). For small samples or extreme
        proportions, consider Wilson or exact methods. Always report both absolute and relative lift
        — relative alone can be misleading.
      </TechNote>
      <Insight>
        The confidence interval is more useful than the point estimate. A 2pp lift with CI [1.5,
        2.5] is much more actionable than a 5pp lift with CI [-1, 11]. Width of the CI reflects how
