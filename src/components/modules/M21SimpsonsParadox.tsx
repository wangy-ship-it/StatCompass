...
export default function M21SimpsonsParadox() {
  ...
  const setSegRatio = (v: number) => set('segRatio', v);
  const setSeg1RateA = (v: number) => set('seg1RateA', v);
  const setSeg1RateB = (v: number) => set('seg1RateB', v);
  ...
  const aggToX = (v: number) => pl + (v / aggMax) * (W - pl - pr);
  const barH = 24;
  const aggY1 = 42;
  ...
  const segToX = (v: number) => pl + (v / segMax) * (W - pl - pr);
  const segBarH = 20;
  const seg1Y = 36;
  const seg2Y = 110;

  const aggTooltipLookup = useCallback(
    (vbX: number) => {
      ...
      const markers = [];
      const aggToX2 = (v: number) => pl + (v / aggMax2) * (W - pl - pr);
      if (Math.abs(vbX - aggToX2(overallA)) < 30)
        markers.push({ y: 42 + 12, color: colors.indigo });
      if (Math.abs(vbX - aggToX2(overallB)) < 30)
        markers.push({ y: 42 + 24 + 20 + 12, color: colors.emerald });
      return { x: vbX, y: 60, lines, markers };
    },
    [overallA, overallB, pl, pr, W],
  );

  const segTooltipLookup = useCallback(
    (vbX: number) => {
      if (vbX < pl || vbX > W - pr) return null;
  ...
  return (
    <div>
      ...
      <Desc>
        A trend in aggregate data can reverse when you split by subgroups. Drag the sliders to
        control segment ratios and success rates, and watch the paradox emerge and vanish in
        real-time. Both views are shown simultaneously so you can see the reversal as it happens.
      </Desc>
      ...
      {paradoxActive && (
        <div
          ...
          style={{ background: sv.fillAmber, border: '1px solid ' + sv.borderAmber }}
        >
          <div
          ...
          <div className="text-sm text-[var(--color-text-primary)] leading-relaxed">
            <span className="font-bold" style={{ color: colors.amber }}>
              Paradox Active
            </span>
            {' \u2014 '}
            {aWinsSeg1 && aWinsSeg2
      ...
      <ChartBox
        h={140}
        tooltipLookup={aggTooltipLookup}
        label="Aggregate view bar chart comparing overall success rates between Group A and Group B"
      >
        <text
          x={W / 2}
          y={16}
        ...
        <rect
          x={pl}
          y={aggY1}
          width={Math.max(aggToX(overallA) - pl, 2)}
          height={barH}
          rx={6}
          fill={colors.indigo}
          opacity={0.75}
        ...
        <rect
          x={pl}
          y={aggY2}
          width={Math.max(aggToX(overallB) - pl, 2)}
          height={barH}
          rx={6}
          fill={colors.emerald}
          opacity={0.75}
      ...
      <ChartBox
        h={180}
        tooltipLookup={segTooltipLookup}
        label="Segmented view showing Group A vs Group B success rates within easy and hard segments separately, revealing the paradox"
      >
        {/* --- Segment 1: Easy --- */}
        <text
          x={pl / 2}
        ...
        <rect
          x={pl}
          y={seg1Y + 10}
          width={Math.max(segToX(s1A) - pl, 2)}
          height={segBarH}
          rx={5}
          fill={colors.indigo}
          opacity={0.7}
        ...
        <rect
          x={pl}
          y={seg1Y + segBarH + 26}
          width={Math.max(segToX(s1B) - pl, 2)}
          height={segBarH}
          rx={5}
          fill={colors.emerald}
          opacity={0.7}
        ...
        <text
          x={segToX(s1B) + 6}
          y={seg1Y + segBarH + 26 + segBarH / 2 + 4}
          fill={colors.emerald}
          fontSize={10}
          fontWeight="700"
        >
        ...
        <rect
          x={pl}
          y={seg2Y + 20}
          width={Math.max(segToX(s2A) - pl, 2)}
          height={segBarH}
          rx={5}
          fill={colors.indigo}
          opacity={0.7}
        ...
        <rect
          x={pl}
          y={seg2Y + segBarH + 36}
          width={Math.max(segToX(s2B) - pl, 2)}
          height={segBarH}
          rx={5}
          fill={colors.emerald}
          opacity={0.7}
        ...
        <text
          x={segToX(s2B) + 6}
          y={seg2Y + segBarH + 36 + segBarH / 2 + 4}
          fill={colors.emerald}
          fontSize={10}
          fontWeight="700"
        >
      ...
      <div className="flex gap-3 mb-6 flex-wrap">
        <StatBox
          label="Overall A"
          value={(overallA * 100).toFixed(1) + '%'}
          color={colors.indigo}
        />
        <StatBox
          label="Overall B"
          value={(overallB * 100).toFixed(1) + '%'}
          color={colors.emerald}
        />
        <StatBox
      ...
      <div className="rounded-xl p-5 mb-6 bg-app-glass ring-1 ring-[var(--color-border-subtle)]">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm text-[var(--color-text-primary)]">
          <div>
            <span className="font-semibold" style={{ color: colors.indigo }}>
              Group A:
            </span>{' '}
      ...
      />
      <TechNote>
        Simpson's Paradox arises from confounding: a lurking variable that influences both the
        grouping and the outcome. It's a special case of omitted variable bias. The key trick here
        is that Group A is disproportionately assigned to the hard segment while Group B gets the
        easy segment, creating an imbalanced comparison. The solution is stratification, matching,
        or regression adjustment to control for confounders.
      </TechNote>
      <Insight>
        Simpson's Paradox is a powerful reminder that aggregate data can be deeply misleading.
        Whenever you see a surprising result, ask: "What if I split this by a key variable?" The
        answer might reverse your conclusion entirely. Use the sliders to find the exact tipping
        point where the paradox appears and disappears.
      </Insight>
    </div>
  );
}
