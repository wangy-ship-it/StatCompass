...
export default function M22RegressionToMean() {
  ...
  const setNUnits = (v: number) => set('nUnits', v);
  ...
  const data = useMemo(() => {
    ...
    const bottomM1Avg =
      sortedIndices.slice(0, bottomCount).reduce((s, d) => s + d.v, 0) / bottomCount;
    const bottomM2Avg =
      sortedIndices.slice(0, bottomCount).reduce((s, d) => s + m2[d.i], 0) / bottomCount;
  ...
  const toX = (v: number) => pl + ((v - mn) / (mx - mn)) * (W - pl - pr);
  const toY = (v: number) => H - pb - ((v - mn) / (mx - mn)) * (H - pt - pb);
  ...
  const tooltipLookup = useCallback(
    (vbX: number) => {
      ...
      const toX2 = (v: number) => pl2 + ((v - mn2) / (mx2 - mn2)) * (W2 - pl2 - pr2);
      const toY2 = (v: number) => 340 - 44 - ((v - mn2) / (mx2 - mn2)) * (340 - 30 - 44);
      let bestIdx = 0,
        bestDist = Infinity;
      ...
      return {
        ...
        lines: [
          {
            label: 'Unit',
            value: '#' + (bestIdx + 1) + (isBottom ? ' (bottom 20%)' : ''),
            color: pointColor,
          },
          ...
          {
            label: 'Change',
            value: (m2Val - m1Val > 0 ? '+' : '') + (m2Val - m1Val).toFixed(1),
            color: m2Val > m1Val ? colors.emerald : colors.red,
          },
        ],
        markers: [{ y: toY2(m2Val), color: pointColor }],
      };
    },
    [data],
  );
  ...
  return (
    <div>
      ...
      <ChartBox
        ...
        label="Scatter plot demonstrating regression to the mean, showing how extreme initial measurements tend to move toward the average on retest"
      >
        {/* Identity line */}
        <line
          x1={toX(mn)}
        ...
        <text x={pl + 26} y={pt + 10} fill={sv.text} fontSize={10}>
          All units
        </text>
        <circle cx={pl + 110} cy={pt + 6} r={6} fill={colors.red} opacity={0.9} />
        <text x={pl + 122} y={pt + 10} fill={colors.red} fontSize={10} fontWeight="600">
          Bottom 20% (M1)
        </text>

        {/* Diagonal label */}
        <text x={toX(mx) - 10} y={toY(mx) + 16} fill={sv.textFaint} fontSize={9} textAnchor="end">
          y = x (no change)
        </text>
      </ChartBox>
      ...
      <div className="flex gap-3 flex-wrap">
        ...
        <StatBox
          label="Bottom 20% M2 Avg"
          value={data.bottomM2Avg.toFixed(1)}
          color={colors.amber}
        />
        <StatBox
          label="Apparent Improvement"
          value={(data.apparentImprovement > 0 ? '+' : '') + data.apparentImprovement.toFixed(1)}
          color={data.apparentImprovement > 0 ? colors.emerald : colors.red}
        />
