...
function calibrate(p: number, calType: string, strength: number) {
...
export default function M24Calibration() {
  ...
  const setCalType = (v: string) => set('calType', v);
  const setStrength = (v: number) => set('strength', v);
  const setNBins = (v: number) => set('nBins', v);
  const setProbe = (v: number) => set('probe', v);
    ...
    for (let b = 0; b < nBins; b++) {
      ...
      if (items.length > 0) {
        ...
        bins.push({
          ...
          actualRate,
          count: items.length,
          gap: Math.abs(meanPred - actualRate),
        });
      }
    }
  ...
  const toY = (v: number) => H - pb - v * (H - pt - pb);
  ...
  const tooltipLookup = useCallback(
    (vbX: number, _vbY: number) => {
      ...
      const gapColor =
        best.gap > 0.15 ? colors.red : best.gap > 0.08 ? colors.amber : colors.indigo;
      return {
        x: toX(mid),
        ...
        lines: [
          {
            label: 'Predicted',
            value: (best.meanPred * 100).toFixed(1) + '%',
            color: colors.amber,
          },
          {
            label: 'Actual',
            value: (best.actualRate * 100).toFixed(1) + '%',
  ...
  return (
    <div>
      ...
      <Desc>
        ...
        predicted probabilities match observed reality. An overconfident model exaggerates —
        predicting 90% when the truth is 70%. Drag the probe slider to test any prediction.
      </Desc>

      <div className="flex gap-2 mb-5 flex-wrap">
        <PillBtn on={calType === 'well'} onClick={() => setCalType('well')}>
          Well Calibrated
        </PillBtn>
        <PillBtn on={calType === 'over'} onClick={() => setCalType('over')}>
          Overconfident
        </PillBtn>
        <PillBtn on={calType === 'under'} onClick={() => setCalType('under')}>
          Underconfident
        </PillBtn>
      </div>
      ...
      <Sl
        ...
        step={0.05}
        onChange={setProbe}
        fmt={(v) => 'Model says ' + (v * 100).toFixed(0) + '%'}
        color={colors.amber}
      />
        ...
        <div className="space-y-2.5">
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-[13px] font-bold" style={{ color: colors.amber }}>
                {(probe * 100).toFixed(0)}%
              </span>
            </div>
            <div
            ...
            >
              <div
                ...
                style={{ width: probe * 100 + '%', background: colors.amber, opacity: 0.7 }}
              />
            </div>
          </div>
          <div>
            <div className="flex items-center justify-between mb-1">
              ...
              <span className="text-[13px] font-bold" style={{ color: colors.emerald }}>
                {(probeActual * 100).toFixed(0)}%
              </span>
            </div>
            <div
              className="h-5 rounded-full overflow-hidden"
            ...
            >
              <div
                className="h-full rounded-full transition-all duration-200"
                style={{ width: probeActual * 100 + '%', background: colors.emerald, opacity: 0.7 }}
              />
            </div>
          </div>
          {probeGap > 0.015 ? (
      ...
      <ChartBox h={H} tooltipLookup={tooltipLookup}>
        ...
        {data.worstBin && data.worstBin.gap > 0.05 && (
          <g>
            ...
            >
              {'Says ' +
                (data.worstBin.meanPred * 100).toFixed(0) +
                '% → Actually ' +
                (data.worstBin.actualRate * 100).toFixed(0) +
                '%'}
            </text>
          </g>
        )}
      ...
      <div className="flex gap-3 flex-wrap mb-5">
        <StatBox
          ...
          color={data.brier > 0.2 ? colors.red : colors.indigo}
        />
        <StatBox
          label="Avg. Gap (ECE)"
          value={(data.ece * 100).toFixed(1) + '%'}
          color={data.ece > 0.1 ? colors.red : colors.emerald}
        />
        <StatBox
          label="Worst Gap"
          value={(data.maxCE * 100).toFixed(0) + '%'}
          color={data.maxCE > 0.15 ? colors.red : colors.amber}
        />
        <StatBox
          label="Type"
          value={
          ...
          color={calType === 'well' ? colors.emerald : colors.amber}
        />
      </div>
