...
export default function M18PracticalSignificance() {
  ...
  const setMwe = (v: number) => set('mwe', v);
  ...
  const toX = (v: number) => pl + ((v + range) / (2 * range)) * (W - pl - pr);
    ...
    (vbX: number) => {
      ...
      return {
        ...
        lines: [
          ...
          {
            label: 'Effect',
            value: (data.effectFrac * 100).toFixed(2) + '%',
            color: colors.indigo,
          },
  ...
  return (
    <div>
      ...
      <Desc>
        A result can be statistically significant but practically meaningless — a tiny effect
        detected with a massive sample. Or it can be practically important but not yet detectable
        with your current data. Always define a Minimum Worthwhile Effect (MWE) before running a
        test.
      </Desc>

      <ChartBox
        h={H}
        label="Comparison of statistical significance versus practical significance, showing minimum detectable effect against business-meaningful threshold"
      ...
      >
        ...
        <line
          ...
          strokeWidth={1.5}
        />
        <text x={zeroX} y={pt - 16} fill={sv.text} fontSize={10} textAnchor="middle">
          0
        </text>
        ...
        {/* CI labels */}
        <text x={ciLoX} y={cy + 26} fill={sv.textFaint} fontSize={9} textAnchor="middle">
          {(data.ciLo * 100).toFixed(2)}%
        </text>
        <text x={ciHiX} y={cy + 26} fill={sv.textFaint} fontSize={9} textAnchor="middle">
          {(data.ciHi * 100).toFixed(2)}%
        </text>
        <text
          x={effectX}
          y={cy - 16}
          ...
          fontWeight="600"
        >
          {(data.effectFrac * 100).toFixed(2)}%
        </text>

        {/* Axis */}
        <line x1={pl} y1={H - pb} x2={W - pr} y2={H - pb} stroke={sv.axis} />
        <text x={W / 2} y={H - 8} fill={sv.textFaint} fontSize={10} textAnchor="middle">
      ...
      <div className="grid grid-cols-2 gap-3 mb-6">
        {quadrantLabels.map((label, i) => (
          <div
            key={i}
            className={
              'rounded-xl px-4 py-3 text-center text-[13px] ring-1 transition-all duration-300 ' +
              (data.quadrant === i
            ...
            style={
              data.quadrant === i
                ? ({
                    ...
                    '--tw-ring-color': quadrantColors[i],
      ...
      />
      <Sl
        label="Min Worthwhile Effect — MWE (%)"
        value={mwe}
        min={0.1}
        max={2}
        step={0.01}
      ...
      <div className="flex gap-3 flex-wrap mt-6">
        <StatBox
          label="p-value"
          value={data.pValue < 0.001 ? '<0.001' : data.pValue.toFixed(4)}
          color={data.significant ? colors.emerald : colors.red}
        />
        <StatBox
          label="CI Width"
          value={(data.ciWidth * 100).toFixed(3) + '%'}
          color={colors.indigo}
        />
