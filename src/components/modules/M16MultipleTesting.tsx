...
export default function M16MultipleTesting() {
  ...
  const setNTests = (v: number) => set('nTests', v);
  const setTrueEffects = (v: number) => set('trueEffects', v);
  const setCorrection = (v: string) => set('correction', v);
  ...
  const data = useMemo(() => {
    ...
    if (correction === 'bonferroni') {
      ...
      significant = pValues.map((pv) => pv < alpha);
    }
  ...
  const toX = (rank: number) => pl + ((rank + 0.5) / sorted.length) * (W - pl - pr);
  const toY = (nlp: number) => H - pb - (nlp / yMax) * (H - pt - pb);

  const fpPrevented =
    correction !== 'none' ? sorted.filter((s) => s.wouldBeSig && !s.sig && !s.isTrue).length : 0;
  ...
  const tooltipLookup = useCallback(
    (vbX: number) => {
      ...
      const barColorVal = s.sig ? (s.isTrue ? colors.emerald : colors.red) : sv.textFaint;
      return {
        x: cx,
        y: cy,
        lines: [
          {
            label: 'p-value',
            value: s.p < 0.0001 ? '<0.0001' : s.p.toFixed(4),
            color: barColorVal,
          },
          {
            label: 'Status',
            value: s.sig ? (s.isTrue ? 'True Positive' : 'False Positive') : 'Not Sig.',
            color: barColorVal,
          },
          {
            label: 'True Effect',
            value: s.isTrue ? 'Yes' : 'No',
            color: s.isTrue ? colors.emerald : sv.textFaint,
          },
        ],
  ...
  return (
    <div>
      ...
      <div className="flex gap-2 mb-5 flex-wrap">
        <PillBtn on={correction === 'none'} onClick={() => setCorrection('none')}>
          No Correction
        </PillBtn>
      ...
      <ChartBox
        h={H}
        label="Multiple testing results showing p-values sorted by significance with correction thresholds"
      ...
      >
        ...
        {correction !== 'bh' && (
          <>
            <line
              x1={pl}
              y1={toY(correction === 'bonferroni' ? negLogThresh : negLogAlpha)}
              x2={W - pr}
              y2={toY(correction === 'bonferroni' ? negLogThresh : negLogAlpha)}
              stroke={colors.amber}
              strokeWidth={2}
              strokeDasharray="8,4"
            />
            <text
              x={pl + 4}
              y={toY(correction === 'bonferroni' ? negLogThresh : negLogAlpha) - 6}
              fill={colors.amber}
              fontSize={9}
              fontWeight="600"
            >
      ...
      <div className="flex items-center justify-end gap-4 mt-2 mb-5">
        <span className="flex items-center gap-1.5">
          <span
            ...
            style={{ background: colors.emerald, opacity: 0.8 }}
          />
          <span className="text-[11px]" style={{ color: sv.text }}>
            True Positive
          </span>
        </span>
        <span className="flex items-center gap-1.5">
          <span
            className="inline-block w-2.5 h-2.5 rounded-sm"
            style={{ background: colors.red, opacity: 0.8 }}
          />
          <span className="text-[11px]" style={{ color: sv.text }}>
            False Positive
          </span>
        ...
        {correction !== 'none' && fpPrevented > 0 && (
          <span className="flex items-center gap-1.5">
            <span
              ...
              style={{ borderColor: colors.red, opacity: 0.6 }}
            />
            <span className="text-[11px]" style={{ color: sv.text }}>
              Caught by correction
            </span>
          </span>
        )}
      </div>
      ...
      <div className="flex gap-3 mb-5 flex-wrap">
        ...
        <StatBox
          label="FWER"
          value={data.fwer ? 'Yes' : 'No'}
          color={data.fwer ? colors.red : colors.emerald}
        />
        <StatBox
          label="FDR"
          value={(data.fdr * 100).toFixed(0) + '%'}
          color={data.fdr > 0.05 ? colors.red : colors.emerald}
        />
      </div>
