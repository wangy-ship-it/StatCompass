...
export default function M25CumulativeGains() {
  ...
  const setModelQuality = (v: number) => set('modelQuality', v);
  const setTargetPct = (v: number) => set('targetPct', v);
  const setBaseRate = (v: number) => set('baseRate', v);
  ...
  const data = useMemo(() => {
    ...
    for (let i = 0; i < nTotal; i++) {
      ...
      if ((i + 1) % step === 0 || i === nTotal - 1) {
        gains.push({
          pct: ((i + 1) / nTotal) * 100,
          captured: nPos > 0 ? (posFound / nPos) * 100 : 0,
        });
      }
    }
    ...
    const atTarget = gains.reduce(
      (best, g) => (Math.abs(g.pct - targetPct) < Math.abs(best.pct - targetPct) ? g : best),
      gains[0],
    );
    ...
    const budgetSavings =
      randomPctNeeded > 0 ? Math.max(0, (1 - targetPct / randomPctNeeded) * 100) : 0;
  ...
  const W = 600,
    ...
    pt = 20,
    pb = 36;
  const toX = (pct: number) => pl + (pct / 100) * (W - pl - pr);
  const toY = (pct: number) => H - pb - (pct / 100) * (H - pt - pb);
  ...
  const tooltipLookup = useCallback(
    (vbX: number, _vbY: number) => {
      ...
      const toX2 = (p: number) => pl2 + (p / 100) * (W2 - pl2 - pr2);
      const toY2 = (p: number) => H2 - pb2 - (p / 100) * (H2 - pt2 - pb2);
      // Find nearest gains point
      let best = data.gains[0],
        bestDist = Infinity;
      ...
      return {
        ...
        lines: [
          ...
          {
            label: 'Lift',
            value: (best.pct > 0 ? (best.captured / best.pct).toFixed(1) : '—') + 'x',
  ...
  return (
    <div>
      ...
      <Desc>
        If you can only contact 20% of your customers, which 20% should you pick? The cumulative
        gains curve shows what fraction of all positive outcomes you capture at each targeting
        level. The further the curve bows above the diagonal, the more value the model adds over
        random selection.
      </Desc>
      ...
      <div className="bg-app-surface rounded-2xl p-5 mb-5 ring-1 ring-[var(--color-border-subtle)]">
        ...
        <div className="space-y-2.5">
          <div>
            ...
            >
              <div
                ...
                style={{ width: targetPct + '%', background: sv.textFaint, opacity: 0.4 }}
              />
            </div>
          </div>
          <div>
          ...
          {data.advantagePP > 2 ? (
            <div
              className="text-center text-[12px] font-semibold pt-1"
              style={{ color: colors.emerald }}
            >
              Model captures {data.advantagePP.toFixed(0)}pp more — your budget works{' '}
              {data.liftAtTarget.toFixed(1)}x harder
            </div>
          ) : (
            <div
              className="text-center text-[12px] font-semibold pt-1"
              style={{ color: sv.textFaint }}
            >
      ...
      <ChartBox
        ...
        label="Cumulative gains curve showing what fraction of positive outcomes is captured at each targeting level compared to random and perfect models"
      >
        <defs>
        ...
        {[0, 25, 50, 75, 100].map((v) => (
          <g key={v}>
            ...
            <text x={toX(v)} y={H - pb + 14} fill={sv.textFaint} fontSize={8} textAnchor="middle">
              {v}%
            </text>
        ...
        <line
          ...
          strokeDasharray="6,4"
        />
        <text x={toX(78)} y={toY(74)} fill={sv.textFaint} fontSize={9} opacity={0.6}>
          Random
        </text>
        ...
        />
        <text
          x={toX(Math.min(50, perfectTurnPct + 8))}
          y={toY(100) + 14}
          fill={colors.emerald}
          fontSize={9}
          opacity={0.6}
        ...
        <line
          x1={pl}
          y1={toY(data.capturedAtTarget)}
          x2={toX(targetPct)}
          y2={toY(data.capturedAtTarget)}
        ...
        <text x={W / 2} y={H - 4} fill={sv.textFaint} fontSize={9} textAnchor="middle">
          % of Customers Targeted (sorted by model score)
        </text>
        <text
          x={14}
          y={(H - pt - pb) / 2 + pt}
      ...
      <div className="flex gap-3 flex-wrap mb-5">
        <StatBox
          label="Captured"
          value={data.capturedAtTarget.toFixed(0) + '%'}
          color={colors.indigo}
        />
        <StatBox
        ...
        <StatBox
          label="vs Random"
          value={'+' + data.advantagePP.toFixed(0) + 'pp'}
          color={colors.emerald}
        />
        <StatBox
          label="Budget Efficiency"
          value={data.liftAtTarget >= 1 ? data.liftAtTarget.toFixed(1) + 'x' : '—'}
          color={data.liftAtTarget >= 2 ? colors.emerald : colors.indigo}
        />
      </div>
      ...
      <Insight>
        This is the most business-friendly model visualization. Unlike AUC or F1, it directly
        answers: "How much more effective is our targeting at a specific budget level?" Use this
        chart in stakeholder meetings to justify model investment in language everyone understands —
        dollars and efficiency, not statistical abstractions.
      </Insight>
    </div>
  );
}
