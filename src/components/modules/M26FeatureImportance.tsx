...
export default function M26FeatureImportance() {
  ...
  const setView = (v: string) => set('view', v);
  const setNFeatures = (v: number) => set('nFeatures', v);
  const setDataPoint = (v: number) => set('dataPoint', v);
  ...
  const data = useMemo(() => {
    ...
    const waterfall: {
      ...
      { name: 'Base value', start: 0, end: baseValue, type: 'base', value: 0, runAfter: baseValue },
    ];
    ind.forEach((f) => {
  ...
  const wfToX = (v: number) => pl + ((v - wfMin) / (wfMax - wfMin)) * (W - pl - pr);
    ...
    (vbX: number, vbY: number) => {
      ...
      if (view === 'global') {
        ...
        return {
          ...
          lines: [
            { label: 'Feature', value: g.name, color: colors.indigo },
            {
              ...
              color: g.positive ? colors.emerald : colors.red,
            },
          ],
          markers: [],
        };
        ...
        const lines: { label: string; value: string; color: string }[] = [
          { label: w.name, value: '', color: colors.indigo },
        ];
        if (w.type !== 'base' && w.type !== 'result') {
          lines.push({
            label: 'Contribution',
            value: (w.value >= 0 ? '+' : '') + w.value.toFixed(3),
            color: w.value >= 0 ? colors.emerald : colors.red,
          });
          lines.push({
            label: 'Running total',
            value: w.runAfter.toFixed(3),
            color: colors.indigo,
          });
        } else if (w.type === 'result') {
          lines.push({
            label: 'Prediction',
            value: (data.prediction * 100).toFixed(1) + '%',
            color: colors.indigo,
          });
        }
        return { x: vbX, y, lines, markers: [] };
      }
    },
  ...
  return (
    <div>
      ...
      <Desc>
        When a model makes a prediction, stakeholders want to know why. Feature importance shows
        which inputs drive the output — like revealing which ingredients matter most in a recipe.
        The global view shows what matters overall; the individual view explains a specific
        prediction.
      </Desc>

      <div className="flex gap-2 mb-5 flex-wrap">
        <PillBtn on={view === 'global'} onClick={() => setView('global')}>
          Global Importance
        </PillBtn>
        <PillBtn on={view === 'individual'} onClick={() => setView('individual')}>
          Explain Prediction
        </PillBtn>
      </div>
              ...
              return (
                <g key={g.name}>
                  ...
                  <rect
                    ...
                    height={rowH - 10}
                    rx={4}
                    fill={g.positive ? colors.emerald : colors.red}
                    opacity={0.7}
                  />
      ...
      <div className="flex gap-3 flex-wrap mb-5">
        ...
        <StatBox
          label="Top Impact"
          value={data.topFeature.importance.toFixed(3)}
          color={colors.indigo}
        />
        {view === 'individual' && (
          <StatBox
      ...
      <TechNote>
        ...
        consistency. They are based on cooperative game theory — each feature's marginal
        contribution averaged over all possible feature orderings.
      </TechNote>
      <Insight>
