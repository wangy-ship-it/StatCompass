...
export default function M28CrossValidation() {
  ...
  const setKFolds = (v: number) => set('kFolds', v);
  const setComplexity = (v: number) => set('complexity', v);
  ...
  const data = useMemo(() => {
    ...
    const cvErrors: number[] = [];
    const cvStds: number[] = [];
    const trainErrors: number[] = [];
    const trueTestErrors: number[] = [];
  ...
  const eToX = (d: number) => pl2 + ((d - 1) / 14) * cw2;
  const eToY = (v: number) => pt2 + ((maxErr - Math.min(v, maxErr)) / maxErr) * ch2;
  ...
  for (let d = 0; d < 15; d++) {
    const x = eToX(d + 1);
    trainErrPath +=
      (d === 0 ? 'M' : 'L') + x.toFixed(1) + ',' + eToY(data.trainErrors[d]).toFixed(1);
    cvErrPath += (d === 0 ? 'M' : 'L') + x.toFixed(1) + ',' + eToY(data.cvErrors[d]).toFixed(1);
    trueTestErrPath +=
      (d === 0 ? 'M' : 'L') + x.toFixed(1) + ',' + eToY(data.trueTestErrors[d]).toFixed(1);
  }
  ...
  const errTooltipLookup = useCallback(
    (vbX: number, _vbY: number) => {
      ...
      const eToX2 = (d: number) => pl2 + ((d - 1) / 14) * cw2;
      const eToY2 = (v: number) => pt2 + ((maxE - Math.min(v, maxE)) / maxE) * ch2;
      return {
        x: eToX2(dIdx + 1),
  ...
  return (
    <div>
      ...
      <Desc>
        ...
        errors. Drag the sliders to see how fold count and model complexity affect the estimate of
      ...
      >
        ...
        {Array.from({ length: kFolds }, (_, f) => {
          ...
          return (
            <g key={'fold-' + f}>
              {/* Fold label */}
              <text x={pl1 - 6} y={y + rowH / 2 + 3.5} fill={sv.text} fontSize={9} textAnchor="end">
                Fold {f + 1}
              </text>
              {/* Segments */}
        ...
        <text x={pl1 + 12} y={H1 - 3} fill={sv.text} fontSize={8}>
          Train
        </text>
        <rect
          x={pl1 + 50}
          ...
          opacity={0.85}
        />
        <text x={pl1 + 62} y={H1 - 3} fill={sv.text} fontSize={8}>
          Test
        </text>
        <text x={W - pr1 + 6} y={H1 - 3} fill={sv.text} fontSize={8} textAnchor="start">
          MSE
        </text>
      </ChartBox>
      ...
      <ChartBox
        ...
        label="Cross-validation results showing training vs test error across folds to assess model generalization"
      >
        {/* Grid lines */}
        {yTicks2.map((t, i) => (
          <line
        ...
        <path d={cvErrPath} fill="none" stroke={colors.amber} strokeWidth={2.5} />
        ...
        <line
          ...
          strokeDasharray="4,3"
        />
        <text x={eToX(data.optDeg)} y={pt2 - 4} fill={sv.text} fontSize={8} textAnchor="middle">
          optimal d={data.optDeg}
        </text>

        {/* Current complexity dots */}
        <circle cx={eToX(complexity)} cy={eToY(data.trainErrors[ci])} r={4} fill={colors.indigo} />
        <circle cx={eToX(complexity)} cy={eToY(data.cvErrors[ci])} r={4} fill={colors.amber} />
        <circle
          cx={eToX(complexity)}
        ...
        {/* Axis titles */}
        <text x={pl2 + cw2 / 2} y={H2 - 2} fill={sv.text} fontSize={9} textAnchor="middle">
        ...
        <line
          ...
          strokeDasharray="4,3"
        />
        <text x={pl2 + cw2 - 112} y={pt2 + 9} fill={colors.indigo} fontSize={8}>
          Training
        </text>
        <line
          x1={pl2 + cw2 - 130}
          ...
          strokeWidth={2.5}
        />
        <text x={pl2 + cw2 - 112} y={pt2 + 22} fill={colors.amber} fontSize={8}>
          CV Error
        </text>
        <line
          x1={pl2 + cw2 - 130}
          ...
          strokeDasharray="4,3"
        />
        <text x={pl2 + cw2 - 112} y={pt2 + 35} fill={colors.emerald} fontSize={8}>
          True Test
        </text>
      </ChartBox>
      ...
      <div className="flex gap-3 mb-5 flex-wrap">
        <StatBox label="Mean CV Error" value={meanCVCurrent.toFixed(4)} color={colors.amber} />
        <StatBox label="CV Std Dev" value={stdCVCurrent.toFixed(4)} color={sv.text} />
        <StatBox label="Optimal Degree" value={data.optDeg} color={colors.emerald} />
        <StatBox label="Training Error" value={trainErrCurrent.toFixed(4)} color={colors.indigo} />
      </div>
      ...
      />
      <TechNote>
        k-fold CV partitions the data into k roughly equal subsets. For each fold f, the model
        trains on the k-1 other folds and predicts the held-out fold. CV error = (1/k) * sum of
        per-fold test errors. The variance of the CV estimate depends on both k and the correlation
        between fold errors. LOOCV (k=n) is nearly unbiased but can have high variance because the
        training sets overlap by n-2 points, making fold errors strongly correlated.
      </TechNote>
      <Insight>
        ...
        sweet spot is typically k=5 to k=10, balancing bias and variance of the error estimate
        itself.
      </Insight>
    </div>
  );
