...
export default function M11AATesting() {
  ...
  const toX = (binIdx: number) => pl + binIdx * (barW + barGap);
  const toY = (count: number) => H - pb - (count / maxCount) * (H - pt - pb);
    ...
    (vbX: number) => {
      ...
      return {
        ...
        lines: [
          {
            label: 'p-value',
            value: binStart.toFixed(2) + '\u2013' + (binStart + binWidth).toFixed(2),
            color: isBelow ? colors.red : colors.indigo,
          },
  ...
  return (
    <div>
      ...
      <Desc>
        ...
        framework is working correctly, p-values should be uniformly distributed and only ~alpha% of
      ...
      >
        ...
        <rect
          ...
          opacity={0.75}
        />
        <text x={W - pr - 116} y={pt + 9} fill={sv.text} fontSize={9}>
          False Positives
        </text>
        <rect
          x={W - pr - 130}
          ...
          opacity={0.5}
        />
        <text x={W - pr - 116} y={pt + 25} fill={sv.text} fontSize={9}>
          Not Significant
        </text>
      </ChartBox>
      ...
      <Insight>
        A/A tests are the immune system of experimentation. Run them before every major experiment
        to confirm your pipeline, randomization, and metrics are healthy. If identical groups show a
