...
export default function M8EffectSizeMDE() {
  ...
  const setBaseline = (v: number) => set('baseline', v);
  const setSampleSize = (v: number) => set('sampleSize', v);
  const setPower = (v: number) => set('power', v);
  ...
  const curvePoints = useMemo(() => {
    ...
    const steps = 100;
    const pts: { delta: number; pw: number }[] = [];
  ...
  const v1toX = (d: number) => v1pl + (d / scaleMax) * (V1W - v1pl - v1pr);
  ...
  const v2toX = (delta: number) => v2pl + (delta / maxDelta) * (V2W - v2pl - v2pr);
  const v2toY = (pw: number) => V2H - v2pb - pw * (V2H - v2pt - v2pb);
  ...
  const underpoweredPath =
    curvePoints
      ...
      .join('') +
    'L' +
    v2toX(curvePoints[safeIdx]?.delta ?? 0) +
    ',' +
    v2toY(0) +
    'L' +
    v2toX(0) +
  ...
  return (
    <div>
      ...
      <ChartBox
        h={V1H}
        label="Effect size visualization showing the overlap between control and treatment distributions with Cohen's d"
      >
        {/* Background bar */}
        <defs>
        ...
        {/* Scale labels */}
        <text x={v1pl} y={V1H - v1pb + 16} fill={sv.text} fontSize={9} textAnchor="middle">
          0
        </text>
        <text
          x={v1toX(scaleMax / 2)}
          ...
          textAnchor="middle"
        >
          {(scaleMax / 2).toFixed(2)}
        </text>
        <text x={v1toX(scaleMax)} y={V1H - v1pb + 16} fill={sv.text} fontSize={9} textAnchor="end">
          {scaleMax.toFixed(2)}
        </text>
        ...
        />
        <text
          x={v1toX(Math.min(computed.d, scaleMax))}
          y={barY - 16}
          fill={colors.indigo}
          fontSize={9}
          textAnchor="middle"
      ...
      <ChartBox
        h={V2H}
        label="Minimum detectable effect curve showing the tradeoff between sample size and the smallest effect that can be reliably detected"
      ...
      <div className="flex gap-3 mb-5 flex-wrap">
        ...
        <StatBox
          ...
          color={colors.emerald}
        />
        <StatBox label="Cohen's d" value={computed.d.toFixed(3)} color={colors.amber} />
        <StatBox label="Required n" value={fmtN(computed.reqN)} color={sv.text} />
      </div>
      ...
      <TechNote>
        ...
        sqrt(2p(1-p)/n), where z_alpha and z_beta are the critical values for significance and power
        respectively. The formula assumes equal group sizes, two-sided testing, and a pooled
