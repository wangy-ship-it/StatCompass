...
export default function M5ExperimentStructure() {
  ...
  const setSplit = (v: number) => set('split', v);
  const setRand = (v: boolean) => set('rand', v);
  const setN = (v: number) => set('N', v);
  ...
  const PW = 600,
    ...
    pT = 16,
    pB = 30;
  const toX = (p: number) => pL + ((p - 10) / 80) * (PW - pL - pR);
  const toY = (v: number) => PH - pB - v * (PH - pT - pB);
  const powerLine = powerPts
    .map((pt, i) => (i === 0 ? 'M' : 'L') + toX(pt.p) + ',' + toY(pt.power))
    .join('');
  ...
  return (
    <div>
      ...
      <div className="flex gap-2.5 mb-5 flex-wrap">
        <PillBtn on={!rand} onClick={() => setRand(false)}>
          Sequential Split
        </PillBtn>
        <PillBtn
          on={rand}
      ...
      <div className="bg-app-surface rounded-2xl p-6 mb-7 ring-1 ring-[var(--color-border-subtle)]">
        <svg viewBox={'0 0 600 ' + svgH} width="100%" style={{ maxHeight: svgH, display: 'block' }}>
          ...
          <text
            x={300}
            y={48}
            fill={rand ? colors.indigo : sv.text}
            fontSize={9}
            textAnchor="middle"
          >
          ...
          <line
            ...
            x2={150}
            y2={58}
            stroke={rand ? colors.indigo : sv.axis}
            strokeWidth={1.5}
          />
          <line
            x1={300}
      ...
      <ChartBox
        h={PH}
        label="Relative statistical power vs control allocation percentage, showing how unequal splits reduce power"
      ...
      <div className="grid grid-cols-2 gap-4 mt-5">
        <div className="rounded-xl bg-app-glass ring-1 ring-[var(--color-border-subtle)] p-4">
          <div className="text-[10px] text-[var(--color-accent-faint)] font-medium uppercase tracking-[0.1em] mb-1.5">
            Primary Metric
          </div>
        ...
        <div className="rounded-xl bg-app-glass ring-1 ring-[var(--color-border-subtle)] p-4">
          <div className="text-[10px] text-[var(--svg-text-faint)] font-medium uppercase tracking-[0.1em] mb-1.5">
            Guardrail Metrics
          </div>
          <div className="text-[13px] text-[var(--svg-text)] leading-relaxed">
      ...
      />
      <TechNote>
        Randomization ensures exchangeability on observed and unobserved covariates. Always hash on
        a stable user ID for consistent assignment across sessions. Consider stratified
        randomization for small samples.
      </TechNote>
      <Insight>
