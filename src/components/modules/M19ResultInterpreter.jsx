...
export default function M19ResultInterpreter() {
  ...
  const stats = useMemo(() => {
    ...
    const direction =
      Math.abs(relLift) < 0.5 ? 'Flat' :
      absLift > 0 ? 'Positive' : 'Negative';

    // Power: probability of detecting the observed effect at alpha=0.05
    const zAlpha = zInv(0.05);
    const power = se > 0
      ? 1 - nCDF(zAlpha - Math.abs(pT - pC) / se, 0, 1)
      : 0;
  ...
  const chart = useMemo(() => {
    ...
    const W = 600, H = 240, pl = 36, pr = 36, pt = 16, pb = 34;
    ...
    const toY = (v) => H - pb - (v / mxY) * (H - pt - pb);
    ...
    if (overlapPts.length > 1) {
      ...
      overlapPts.forEach((p) => (overlapPath += 'L' + p.x + ',' + p.y));
      overlapPath += 'L' + overlapPts[overlapPts.length - 1].x + ',' + overlapPts[overlapPts.length - 1].b + 'Z';
    }
    ...
    return {
      pathC, pathT, areaC, areaT, overlapPath,
      toX, toY, W, H, pl, pr, pt, pb, ticks,
    };
  ...
  const applyPreset = (p) => {
    setControlRate(p.controlRate);
    setTreatmentRate(p.treatmentRate);
  ...
  return (
    <div>
      ...
      <Desc>
        After running an experiment, you need to interpret the results and decide what to do.
        Drag the sliders to explore how control rate, treatment rate, and sample size interact
        to shape your conclusion — or pick a preset scenario to jump to a classic outcome.
      </Desc>

      {/* ── Preset pills ── */}
      <div className="flex gap-2 mb-5 flex-wrap">
        {presets.map((p, i) => (
          <PillBtn key={p.label} on={activePreset === i} onClick={() => applyPreset(p)}>
            {p.label}
          </PillBtn>
        ))}
      </div>
      ...
      <ChartBox h={chart.H} label="Experiment result visualization showing confidence interval, point estimate, and significance assessment">
        <defs>
          ...
          <pattern id="hatch-overlap" width="6" height="6" patternUnits="userSpaceOnUse" patternTransform="rotate(45)">
            <line x1="0" y1="0" x2="0" y2="6" stroke={colors.indigo} strokeWidth="1.2" opacity="0.25" />
          </pattern>
        </defs>
        {/* Shaded fills */}
        ...
        {/* Mean dashed lines */}
        <line
          x1={chart.toX(controlRate)} y1={chart.pt}
          x2={chart.toX(controlRate)} y2={chart.H - chart.pb}
          stroke={colors.indigo} strokeWidth={1.5} strokeDasharray="5,3"
        />
        <line
          x1={chart.toX(treatmentRate)} y1={chart.pt}
          x2={chart.toX(treatmentRate)} y2={chart.H - chart.pb}
          stroke={colors.emerald} strokeWidth={1.5} strokeDasharray="5,3"
        />

        {/* Mean labels */}
        <text
          x={chart.toX(controlRate)} y={12}
          fill={colors.indigo} fontSize={9} textAnchor="middle" fontWeight="600"
        >
          {'Control: ' + controlRate.toFixed(1) + '%'}
        </text>
        <text
          x={chart.toX(treatmentRate)} y={12}
          fill={colors.emerald} fontSize={9} textAnchor="middle" fontWeight="600"
        >
        ...
        {/* X-axis */}
        <line
          x1={chart.pl} y1={chart.H - chart.pb}
        ...
        {chart.ticks.map((t, i) => (
          <g key={i}>
            <line
              x1={t.x} y1={chart.H - chart.pb}
              x2={t.x} y2={chart.H - chart.pb + 4}
              stroke={sv.axis}
            />
            <text
              x={t.x} y={chart.H - chart.pb + 14}
              fill={sv.text} fontSize={8} textAnchor="middle"
            >
              {t.label}
            </text>
          </g>
        ...
        {/* Axis label */}
        <text
          x={chart.W / 2} y={chart.H - 2}
          fill={sv.text} fontSize={9} textAnchor="middle"
        >
          Conversion Rate (%)
        </text>
      </ChartBox>
      ...
      <div className="rounded-xl p-5 mb-5 transition-all duration-300" style={{ background: stats.recBgVar, border: '1px solid ' + stats.recBorderVar }}>
        ...
        <div className="text-sm text-[var(--color-text-primary)] leading-relaxed">{stats.recText}</div>
      </div>
