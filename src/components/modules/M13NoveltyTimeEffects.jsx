...
function effectCurve(type, magnitude, decayRate, week) {
...
export default function M13NoveltyTimeEffects() {
  ...
  const data = useMemo(() => {
    ...
    const measurementBias = peakVal > 0
      ? (Math.abs(initial.current - steady.steadyState) / peakVal) * 100
      : 0;
  ...
  const W = 600, H = 240, pl = 56, pr = 30, pt = 30, pb = 40;
  const maxEffect = Math.max(magnitude * 1.2, 5);
  const toX = (w) => pl + (w / 12) * (W - pl - pr);
  const toY = (e) => H - pb - (e / maxEffect) * (H - pt - pb);

  let curvePath = '';
  data.points.forEach((p, i) => {
    curvePath += (i === 0 ? 'M' : 'L') + toX(p.week) + ',' + toY(p.effect);
  });

  let fillPath = `M${toX(0)},${toY(0)}`;
  data.points.forEach((p) => { fillPath += `L${toX(p.week)},${toY(p.effect)}`; });
  ...
  return (
    <div>
      ...
      <Desc>
        ...
        change initially. Understanding these dynamics is critical for reading experiment results correctly.
      </Desc>

      <div className="flex gap-2 mb-6 flex-wrap">
        {Object.entries(effectTypes).map(([key, { label }]) => (
          <PillBtn key={key} on={effectType === key} onClick={() => setEffectType(key)}>{label}</PillBtn>
        ))}
      </div>
      ...
      <ChartBox h={H} label="Time series showing how novelty and primacy effects cause treatment impact to change over the experiment duration">
        ...
        {[0, 2, 4, 6, 8, 10, 12].map((w) => (
          <g key={w}>
            ...
            <text x={toX(w)} y={H - pb + 14} fill={sv.textFaint} fontSize={9} textAnchor="middle">W{w}</text>
          </g>
        ))}

        {/* Steady state dashed line */}
        <line x1={pl} y1={toY(data.steadyState)} x2={W - pr} y2={toY(data.steadyState)} stroke={curveColor} strokeWidth={1.5} strokeDasharray="6,4" opacity={0.6} />
        <text x={W - pr + 4} y={toY(data.steadyState) - 4} fill={curveColor} fontSize={9} opacity={0.8}>Steady state</text>

        {/* Observation window marker */}
        <line x1={toX(window)} y1={pt} x2={toX(window)} y2={H - pb} stroke={colors.red} strokeWidth={1.5} strokeDasharray="4,3" opacity={0.7} />
        ...
        <text x={toX(window) + 10} y={toY(data.atWindowEffect) - 4} fill={curveColor} fontSize={10} fontWeight="600">
        ...
        <text x={W / 2} y={H - 6} fill={sv.textFaint} fontSize={10} textAnchor="middle">Weeks</text>
        <text x={14} y={(H - pt - pb) / 2 + pt} fill={sv.textFaint} fontSize={10} textAnchor="middle" transform={`rotate(-90,14,${(H - pt - pb) / 2 + pt})`}>Effect %</text>
      </ChartBox>

      <Sl label="Effect Magnitude (initial lift %)" value={magnitude} min={1} max={30} step={0.5} onChange={setMagnitude} fmt={(v) => v.toFixed(1) + '%'} color={curveColor} />
      <Sl label="Decay / Adaptation Rate" value={decayRate} min={0.05} max={1} step={0.05} onChange={setDecayRate} fmt={(v) => v.toFixed(2)} color={colors.amber} />
      <Sl label="Observation Window (weeks)" value={window} min={1} max={12} step={0.5} onChange={setWindow} fmt={(v) => v.toFixed(1) + ' wk'} color={colors.red} />

      <div className="flex gap-3 flex-wrap mt-6">
        <StatBox label="Initial Effect" value={data.initialEffect.toFixed(1) + '%'} color={curveColor} />
        <StatBox label="Steady-State" value={data.steadyState.toFixed(1) + '%'} color={colors.emerald} />
        <StatBox label="Measurement Bias" value={data.measurementBias.toFixed(0) + '%'} color={data.measurementBias > 20 ? colors.red : colors.amber} />
