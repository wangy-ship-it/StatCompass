...
export default function M4BayesianFrequentist() {
  ...
  const setObserved = (v: number) => set('observed', v);
  const trueRate = 0.25;
  ...
  const priorPts: { x: number; yPr: number; yPo: number }[] = [];
  let maxY = 0;
  for (let i = 0; i <= 200; i++) {
  ...
  const fW = 280,
    fH = 180;
  const fToX = (v: number) => pl + ((v - xMin) / (xMax - xMin)) * (fW - pl - pr);
  ...
  const fToY = (v: number) => fH - pb - (v / (fMaxY || 1)) * (fH - pt - pb);
  let fPath = '',
    fArea = '';
  fPts.forEach((p, i) => {
  ...
  return (
    <div>
      ...
      <Desc>
        Two philosophies, one question. Frequentists ask "how surprising is this data if H0 is
        true?" Bayesians ask "given this data, what do I believe now?" Watch how prior beliefs and
        data combine to form the posterior — and how it converges with the frequentist answer as
        data grows.
      </Desc>
      ...
      <div className="flex gap-4 flex-wrap mb-5">
        ...
        <div className="flex-1 min-w-[260px] bg-app-surface rounded-2xl p-6 ring-1 ring-[var(--color-border-subtle)]">
          <div className="text-[11px] text-[var(--svg-text)] uppercase tracking-widest mb-2 font-bold text-center">
            Bayesian
          </div>
          <svg
            viewBox={'0 0 ' + W + ' ' + H}
            width="100%"
            style={{ maxHeight: H, display: 'block', overflow: 'visible' }}
          >
            ...
            <line
              x1={toX(data.postMean)}
              y1={pt}
              x2={toX(data.postMean)}
              y2={H - pb}
            ...
            <text x={pl + 4} y={pt - 4} fill={sv.text} fontSize={8}>
              Prior (dashed)
            </text>
            <text
              x={W - pr - 4}
          ...
          <div className="text-center mt-2">
            ...
            <span className="text-sm font-bold font-mono" style={{ color: colors.indigo }}>
              {data.postMean.toFixed(3)}
            </span>
            <span className="text-xs text-[var(--svg-text)] ml-3">95% credible: </span>
            <span className="text-xs font-mono text-[var(--svg-text)]">
              [{data.bayesCI[0].toFixed(3)}, {data.bayesCI[1].toFixed(3)}]
            </span>
          </div>
        </div>
        ...
        <div className="flex-1 min-w-[260px] bg-app-surface rounded-2xl p-6 ring-1 ring-[var(--color-border-subtle)]">
          ...
          <svg
            viewBox={'0 0 ' + fW + ' ' + fH}
            width="100%"
            style={{ maxHeight: fH, display: 'block', overflow: 'visible' }}
          >
            ...
            {/* CI bar */}
            <line
              x1={fToX(data.freqCI[0])}
              y1={fH - pb - 10}
              x2={fToX(data.freqCI[1])}
              y2={fH - pb - 10}
              stroke={colors.emerald}
              strokeWidth={3}
              strokeLinecap="round"
          ...
          <div className="text-center mt-2">
            ...
            <span className="text-sm font-bold font-mono" style={{ color: colors.emerald }}>
              {data.pHat.toFixed(3)}
            </span>
            <span className="text-xs text-[var(--svg-text)] ml-3">95% CI: </span>
            <span className="text-xs font-mono text-[var(--svg-text)]">
              [{data.freqCI[0].toFixed(3)}, {data.freqCI[1].toFixed(3)}]
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-3 mb-5 flex-wrap">
        ...
        <StatBox
          label="P-value (H0: p=0.2)"
          value={pVal < 0.0001 ? '< 0.0001' : pVal.toFixed(4)}
          color={sig ? colors.emerald : sv.textFaint}
        />
      </div>
      ...
      <Sl
        label="Observed Data (n)"
        value={observed}
        min={10}
        max={1000}
        step={10}
        onChange={setObserved}
        fmt={(v: number) => v + ' observations'}
        color={colors.indigo}
      />
      ...
      <TechNote>
        ...
        posterior. This is exact, not an approximation. The posterior mean is a weighted average of
        the prior mean and the MLE, with weights proportional to the effective sample sizes.
      </TechNote>
      <Insight>
        With enough data, the prior doesn't matter — the posterior converges to the true value
        regardless of where you started. This is why the Bayesian vs Frequentist debate matters less
        in practice than in philosophy: both work well with sufficient data.
      </Insight>
    </div>
  );
}
