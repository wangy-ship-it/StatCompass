...
export default function M2PValueTesting() {
  ...
  const W = 600, H = 220, pl = 36, pr = 36, pt = 16, pb = 36;
  const toX = (v) => pl + ((v + 4) / 8) * (W - pl - pr);
  const toY = (v) => H - pb - (v / 0.4) * (H - pt - pb);
  ...
  return (
    <div>
      ...
      <ChartBox h={H} label="Normal distribution with shaded p-value region showing probability of observing results as extreme as the test statistic" tooltipLookup={tooltipLookup}>
        ...
        <line x1={toX(ts)} y1={pt} x2={toX(ts)} y2={toY(0)} stroke={colors.indigo} strokeWidth={2} strokeDasharray="5,3" />
        {two && ts !== 0 && (
          <line x1={toX(-ts)} y1={pt} x2={toX(-ts)} y2={toY(0)} stroke={colors.indigo} strokeWidth={2} strokeDasharray="5,3" opacity={0.4} />
        )}
      ...
      <Sl label="Test Statistic (Z-score)" value={ts} min={-3.5} max={3.5} step={0.05} onChange={setTs} fmt={(v) => v.toFixed(2)} color={colors.indigo} />

      <div className="flex gap-3 mb-4">
        <PillBtn on={two} onClick={() => setTwo(true)}>Two-tailed</PillBtn>
        <PillBtn on={!two} onClick={() => setTwo(false)}>One-tailed</PillBtn>
      </div>

      <div className="text-[13px] text-[var(--svg-text)] font-bold mb-3">Which test should I use?</div>
      <div className="grid grid-cols-2 gap-3">
        {[
          ...
          <div key={i} className="rounded-xl bg-app-glass p-4">
            <div className="text-[13px] text-[var(--color-text-primary)] font-semibold">{t.icon + ' ' + t.name}</div>
