...
export default function M3ConfidenceIntervals() {
  ...
  const W = 600, H = 340, pl = 36, pr = 36, pt = 20, ppb = 20;
    ...
    <div>
      ...
      <ChartBox h={H} label="Twenty-five confidence intervals from repeated samples, showing how most capture the true value while some miss">
        ...
        {ivs.map((ci, i) => {
          ...
          return (
            <g key={i}>
              <line x1={toX(ci.lo)} y1={y} x2={toX(ci.hi)} y2={y} stroke={c} strokeWidth={2.5} strokeLinecap="round" opacity={0.7} />
      ...
      <Sl label="Confidence Level" value={cl} min={0.5} max={0.99} step={0.01} onChange={setCl} fmt={(v) => Math.round(v * 100) + '%'} color={colors.indigo} />
      <Sl label="Sample Size" value={ss} min={5} max={200} step={1} onChange={setSs} color={colors.emerald} />
