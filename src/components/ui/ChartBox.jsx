...
export default function ChartBox({ children, h, w, label, tooltipLookup, className }) {
  const { svgRef, tip, onPointerMove, onPointerLeave } = useChartTooltip(tooltipLookup);
  return (
    <div className={className || "backdrop-blur-xl bg-app-glass rounded-3xl p-4 sm:p-8 mb-8 ring-1 ring-[var(--color-border-subtle)]"}>
      <svg
